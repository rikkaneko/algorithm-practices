...
namespace nnplib {
    void mergesort::merge(std::vector<double> &arr, std::vector<double> &aux, int lo, int mid, int hi) {
    ...
    void mergesort::sort(std::vector<double> &arr, std::vector<double> &aux, int lo, int hi) {
        if (lo >= hi) return;
    ...
    void mergesort::sort(std::vector<double> &arr, int st, int ed) {
        if (ed == -1) ed = arr.size();
        // optimization #1: use pre-allocated array for buffer.
        std::vector<double> aux(arr.size());
        sort(arr, aux, st, ed - 1);
    }
    
    void mergesort::sort_v2(std::vector<double> &arr, int st, int ed) {
        if (ed == -1) ed = arr.size();
        int n = ed - st;
        std::vector<double> aux(arr.size());
