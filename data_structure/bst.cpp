...
namespace nnplib {
    void bst::insert(const std::string &key, const std::string &val) {
        root_ = insert(root_, key, val);
    }
    
    const bst::node *bst::find(const std::string &key) const {
    ...
    std::optional<bst::node> bst::remove_min() {
    ...
    std::optional<bst::node> bst::remove_max() {
    ...
    bst::node *bst::insert(node *root, const std::string &key, const std::string &val) {
        if (!root) {
            ++count_;
    ...
    bst::node *bst::find(bst::node *root, const std::string &key) {
    ...
    bst::node *bst::remove(node *root, const std::string &key) {
        if (!root) return nullptr;
        node *succ = root;
    ...
    std::queue<const bst::node *> bst::inorder() const {
        std::queue<const node *> ordered;
        inorder(root_, ordered);
        return ordered;
    }
    
    void bst::inorder(bst::node *root, std::queue<const node *> &nodes) {
        if (!root) return;
