...
namespace nnplib {
    const rbt::node *rbt::find(const std::string &key) const {
        return find(root_, key);
    }
    
    rbt::node *rbt::find(rbt::node *root, const std::string &key) {
    ...
    void rbt::insert(const std::string &key, const std::string &val) {
        root_ = insert(root_, key, val);
    }
    
    rbt::node *rbt::insert(rbt::node *root, const std::string &key, const std::string &val) {
        if (!root) {
            ++count_;
    ...
    void rbt::flip_color(rbt::node *root) {
        ...
        root->left->col = node::black;
        root->right->col = node::black;
    }
    
    bool rbt::is_red(rbt::node *root) {
        if (!root) return false;
        return root->col == node::red;
    }
}
